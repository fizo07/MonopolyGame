...
public class MultiplayerBoard implements ActionListener {

    private JFrame f;
    private JPanel Board,  Center,  Street[],  Deed,  bg,  Deedstatus,  Actionpanel,  PlayerInfo,  CurPlayers,  Dicepanel,  NoTurn,  chatpanel;
    //JButton Move;
    private JLabel timelabel,  othertime;
    private JLabel Detail,  ownerlabel,  statuslabel,  rentlabel,  houselabel,  costlabel,  owner,  status,  rent,  house,  cost;//status labels
    private JButton build,  sell,  mortgage,  unmortgage,  trade;
    private ImageIcon bgicon;
    private JTextArea history;
    public JTextArea log;
    private JTabbedPane infotab,  playertab;
    public Cell Cell[],  com[];
    private JPanel houseimage[];
    public Player Player[];
    public Bank bank;
    private Sound sound;
    private Toolkit toolkit = java.awt.Toolkit.getDefaultToolkit();
    private Dice DiceControl;
    boolean throwable = true;
    private int Delay = 100;
    private Timer Runtimer;
    private int turn = 0;
    private int goMoney = 200;
    private MultiplayerSetup client;
    private ChatterClient chat;
    private boolean isTurn = false;
    private Time2 timer2;
    private Media media;
    public Options options;
    int help = 1;

    public MultiplayerBoard(Player Player[], MultiplayerSetup client, ChatterClient chat, Options options) {
        this.options = options;
        this.client = client;
        //this.chat = chat;
        this.Player = Player;
        this.turn = client.turn;
        f = new JFrame();
        Board = new JPanel();
        Center = new JPanel();
        ...
        Deed = new JPanel();
        sound = new Sound();
        timer2 = new Time2(new Time(0, 2, 0));
        log = new JTextArea();
    ...
    private void arrangePanels() {
        ...
        String dir = "images/street_images/";
        int x = 9;
        int y = 0;
        int a = 9;
        int b = 0;
        for (int i = 0; i < 40; i++) {
            ...
            if (i == 0) {
                ...
                Street[i].setBackground(Color.yellow);
                //addImage(Street[i],dir+BoardComponent.images[i],(int)BoardDimension.getWidth(d.height),(int)BoardDimension.getWidth(d.height));
            ...
            if (i > 0 && i < 10) {
                ...
                Street[i].setBackground(Color.blue);
                houseimage[i].setBackground(Color.black);
                ...
                //addImage(Street[i],dir+BoardComponent.images[i],(int)BoardDimension.getHeight(d.height),(int)BoardDimension.getWidth(d.height));
                Street[i].setLayout(new GridLayout(1, 1));
                Street[i].add(Cell[i]);
            }
            if (i > 10 && i < 20) {
                ...
                Street[i].setBackground(Color.red);
                houseimage[i].setBackground(Color.blue);
                ...
                //addImage(Street[i],dir+BoardComponent.images[i],(int)BoardDimension.getWidth(d.height),(int)BoardDimension.getHeight(d.height));
                Street[i].setLayout(new GridLayout(1, 1));
                Street[i].add(Cell[i]);
            }
            if (i > 20 && i < 30) {
                ...
                Street[i].setBackground(Color.green);
                houseimage[i].setBackground(Color.black);
...
// 	}
    ...
    private JPanel ChatPanel() {
        ...
        send.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                client.sendMessage("#-MESSAGECHAT-" + Player[turn].getName() + "-" + type.getText().trim() + "-*");
                log.append(Player[turn].getName() + " >>> " + type.getText().trim() + "\n");
            }
        });
        //aaaaaaaaaaaaaaaaaaaaaa
        p.setLayout(null);
        chatpanel.setBounds(25, 10, 350, 350);
        p.add(chatpanel);
        panel.add(p);
        panel.setLayout(new GridLayout(1, 1));
                ...
                public void mouseClicked(MouseEvent me) {
                    if (b) {
                        ...
                        if (com[z].getOwner() == Player[turn]) {
                            v1.remove(Cell[z]);
                            com[z].removeAll();
                            addImage(com[z], dir + BoardComponent.Deeds[z], 30, 30);
                            com[z].revalidate();
                            b = true;
                        }
                    }
                ...
                public void mouseClicked(MouseEvent me) {
                    if (b) {
                        ...
                        if (com1[z].getOwner() == trader) {
                            v2.remove(Cell[z]);
                            com1[z].removeAll();
                            addImage(com1[z], dir + BoardComponent.Deeds[z], 30, 30);
                            com1[z].revalidate();
                            b = true;
                        }
                    }
        ...
        offer.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (v1.size() != 0 || v2.size() != 0) {
                    ...
                    client.sendMessage("#-TRADE-" + turn + "-" + model.getNumber().intValue() + "-" + sa + "-" + pno + "-" + model1.getNumber().intValue() + "-" + sb + "-*");
    ...
    public void makeTrade() {
        ...
        title.setFont(new Font("cooper black", 0, 25));
    ...
    private void buildHouse(final int current) {
        ...
        final JButton sell = new JButton(new ImageIcon(getClass().getResource("images/sellhouse.png")));
        ...
        for (int i = 0; i < com.length; i++) {
            ...
            com[i].addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent me) {
                    ...
                    cost.setText("Cost of House - $" + String.valueOf(Cell[z].getCostOfHouse()));
                    ...
                    if (com[no].getComponentCount() < 1) {
                        ...
                        if (Cell[no].getNumberOfHouses() <= 5) {
                            if (Player[current].getMoney() >= Cell[no].getCostOfHouse()) {
                                ...
                                client.sendMessage("#-PURCHASEHOUSE-" + current + "-" + no + "-*");
                                try {
                                    Thread.sleep(500);
                                } catch (Exception ae) {
                                }
                                ...
                                updateMessage(Player[current].getName() + " built a house on " + Cell[no].getName());
                                sound.playBuild();
                                Cell[no].revalidate();
                                Cell[no].repaint();
                                trade.setVisible(false);
        ...
        sell.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (v.isEmpty()) {
                    ...
                    client.sendMessage("#-SELLHOUSE-" + current + "-" + no + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
                    ...
                    updateMessage(Player[current].getName() + " sold a house on " + Cell[no].getName());
                    sound.playBuild();
                    Cell[no].revalidate();
                    Cell[no].repaint();
                    trade.setVisible(false);
        ...
        sell.setBounds(230, 230, 100, 50);
        name.setBounds(220, 50, 120, 20);
        cost.setBounds(220, 70, 120, 20);
                ...
                if (v.isEmpty()) {
                    ...
                    if (Cell[no].isMortgaged()) {
                        ...
                        if (Cell[no].getNumberOfHouses() > 0) {
                            ...
                            client.sendMessage("#-MORTGAGE-" + no + "-*");
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
                            ...
                            sound.playMoney();
                            client.sendMessage("#-PAYPLAYER-" + turn + "-" + Cell[no].getCost() / 2 + "-*");
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
                ...
                if (v.isEmpty()) {
                    ...
                    if (!Cell[no].isMortgaged()) {
                        ...
                        if (value <= Player[turn].getMoney()) {
                            ...
                            client.sendMessage("#-UNMORTGAGE-" + no + "-*");
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
                            ...
                            sound.playCash();
                            client.sendMessage("#-PAYBANK-" + turn + "-" + value + "-*");
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
...
////////////////////
    ...
    public void DicePanel() {
        ...
        final JLabel dice1 = new JLabel(new ImageIcon(Dice.class.getResource("/images/dice/0.png")));
        final JLabel dice2 = new JLabel(new ImageIcon(Dice.class.getResource("/images/dice/0.png")));
        final JButton button = new JButton(new ImageIcon(getClass().getResource("images/Spinner.gif")));
        ...
        final JLabel text = new JLabel("Total: Nil");
        final JLabel nametext = new JLabel(Player[turn].getName() + " to roll", Player[turn].token.getIcon(), JLabel.CENTER);
        Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
        Font font = new Font("cooper black", Font.BOLD, 13);
        nametext.setFont(font);
        nametext.setForeground(Color.BLACK);
        move.setEnabled(false);
        move.setToolTipText("Click here to move player token!");
        button.setToolTipText("Click here to roll Dice!");
        endturn.setFont(new Font("cooper black", Font.BOLD, 25));
        endturn.setBackground(Color.WHITE);
        endturn.setBounds(25, 15, 100, 60);
        Dicepanel.setBackground(Color.red);
        Dicepanel.setBounds((int) (Board.getWidth() * 0.5) - 75, (int) (Board.getWidth() * 0.65), 150, 135);
        ...
        Dicepanel.setBorder(raisedetched);
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (throwable) {
                    try {
                        ThrowDice throwing = new ThrowDice(dice1, dice2, text, DiceControl);
                        throwing.start();
                    } catch (Exception ae) {
        ...
        endturn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Dicepanel.updateUI();
                endTurn();
                nametext.setText(Player[turn].getName() + " to roll");
                nametext.setIcon(Player[turn].token.getIcon());
                //sound.playDiceSK();
...
//            }
    ...
    public void otherTurn(int other) {
        for (int i = 0; i < bg.getComponents().length; i++) {
            if (bg.getComponents()[i] == NoTurn) {
                bg.remove(NoTurn);
            }
        }
        NoTurn = new JPanel();
        JLabel otherplayer = new JLabel(Player[other].getName(), Player[other].token.getIcon(), JLabel.CENTER);
        othertime = new JLabel(" TIME--> 00:00:00");
        Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
        othertime.setBorder(raisedetched);
        otherplayer.setBorder(raisedetched);
        NoTurn.add(othertime);
        NoTurn.add(otherplayer);
        NoTurn.setLayout(null);
        NoTurn.setBounds((int) (Board.getWidth() * 0.5) - 75, (int) (Board.getWidth() * 0.65), 150, 135);
        othertime.setBounds(0, 0, 150, 50);
        otherplayer.setBounds(0, 50, 150, 85);
        bg.add(NoTurn);
        if (turn == other) {
    ...
    private void endTurn() {
        ...
        client.sendMessage("#-ENDTURN-" + turn + "-*");
        //bg.remove(DicePanel());
        bg.validate();
        bg.revalidate();
        bg.repaint();
    ...
    public void updatePlayerDetails() {
        client.sendMessage("#-UPDATEPLAYERDETAILS" + "-*");
        try {
            Thread.sleep(1000);
        } catch (Exception ae) {
        }
    ...
    public void updatePlayerDetails2() {
        SwingUtilities.invokeLater(
                new Runnable() {
    ...
    public void updateMessage2(String s) {
        history.append(timelabel.getText() + " - " + s + "\n" + "-------------------------------------------------------------------------" + "\n");
    }

    public void tradeProperty(final Player trader, final int tradercash, final Cell traderproperty[], final Player tradee, final int tradeecash, final Cell tradeeproperty[]) {
        ...
        JButton can = new JButton("Decline");
    ...
    private void movePlayer(int dice, int position) {
        ...
        if (Player[turn].inJail()) {
            ...
            if (Player[turn].getJailCount() < 2) {
                JOptionPane.showMessageDialog(null, Player[turn].getJailCount());
                playInJail();
            } else {
                ...
                sound.playCash();
                client.sendMessage("#-PAYBANK-" + turn + "-" + 50 + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
                Player[turn].setInJail(false);
                client.sendMessage("#-OUTJAIL-" + turn + "-*");
            }
        }
        if (!Player[turn].inJail()) {
            if (dice != 0) {
                if (newpos < 40) {
                    ...
                    client.sendMessage("#-MOVEPLAYER-" + turn + "-" + newpos + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
                ...
                client.sendMessage("#-MOVEPLAYER-" + turn + "-" + position + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
            ...
            if (Cell[newpos].isAvailable()) {
                if (Cell[newpos].getOwner() != Player[turn] && Cell[newpos].getOwner() == null) {
                    //Option buy house.
                    //JOptionPane.showMessageDialog(null,"Can buy property!");
                    ...
                    //pay rent.
                    //JOptionPane.showMessageDialog(null,"You must pay rent!");
                ...
                if (newpos == 2 || newpos == 17 || newpos == 33) {
                    //JOptionPane.showMessageDialog(null,"Community Chest");
                    playCommunityChest();
                } else if (newpos == 7 || newpos == 22 || newpos == 36) {
                    //JOptionPane.showMessageDialog(null,"Chance");
                    playChance();
                } else if (newpos == 10) {
                    //JOptionPane.showMessageDialog(null,"Just visiting jail");
                    playJustVisiting();
                } else if (newpos == 20) {
                    ...
                    //JOptionPane.showMessageDialog(null,"Income tax");
                    playIncomeTax();
                } else if (newpos == 38) {
                    //JOptionPane.showMessageDialog(null,"Super tax");
                    playSuperTax();
                }
            }
        }
    ...
    private void buyProperty(final int pos, boolean auction) {
        ...
        JButton auc = new JButton("No Thanks");
        ...
        buy.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (Player[turn].getMoney() >= Cell[pos].getCost()) {
                    ...
                    //try{Thread.sleep(500);}catch(Exception ae){}
                    client.sendMessage("#-PURCHASEPROPERTY-" + turn + "-" + pos + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
        ...
        ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (s.equals("railroad")) {
                    ...
                    sound.playCash();
                    client.sendMessage("#-PAYRENT-RAIL-" + turn + "-" + i + "-NIL" + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
                    ...
                    sound.playCash();
                    client.sendMessage("#-PAYRENT-UTIL-" + turn + "-" + i + "-" + dice + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
                    ...
                    sound.playCash();
                    client.sendMessage("#-PAYRENT-NORM-" + turn + "-" + i + "-NIL" + "-*");
                    try {
                        Thread.sleep(500);
                    } catch (Exception ae) {
                    }
        ...
        new Runnable() {
            ...
            public void run() {
                ...
                ok.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        Idialog.setVisible(false);
                        bank.payPlayer(Player[i], goMoney);
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + goMoney + "-*");
    ...
    private void playChance() {
        ...
        JPanel Cback = new JPanel() {
            ...
            public void paintComponent(Graphics g) {
                g.drawImage(new ImageIcon(getClass().getResource("images/back4.png")).getImage(), 0, 0, 300, 250, null);
                super.paintComponent(g);
            }
        };
        Cback.setOpaque(false);
        ...
        drawcard.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                ok.setEnabled(true);
                client.sendMessage("#-CARDDRAWN-" + turn + "-" + z + "-" + 1 + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (z) {
                    case 1:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 50 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 2:
                        ...
                        sound.playCash();
                        client.sendMessage("#-PAYBANK-" + turn + "-" + (Player[turn].getNoOfHouses() * 25) + (Player[turn].getNoOfHotels() * 100) + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 3:
                        ...
                        client.sendMessage("#-AQUIREDCHANCECARD-" + turn + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 10:
                        ...
                        for (int i = 0; i < Player.length; i++) {
                            if (i != turn) {
                                ...
                                client.sendMessage("#-PAYPLAYER-" + i + "-" + 50 + "-*");
                                sound.playCash();
                                try {
                                    Thread.sleep(500);
                                } catch (Exception ae) {
                    ...
                    case 13:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 150 + "-*");
                        try {
                            Thread.sleep(1000);
                        } catch (Exception ae) {
                        }
                    ...
                    case 16:
                        ...
                        sound.playCash();
                        client.sendMessage("#-PAYBANK-" + turn + "-" + 15 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
    ...
    private void playCommunityChest() {
        ...
        drawcard.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                drawcard.setEnabled(false);
                ok.setEnabled(true);
                client.sendMessage("#-CARDDRAWN-" + turn + "-" + z + "-" + 2 + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (z) {
                    case 1:
                        ...
                        sound.playCash();
                        client.sendMessage("#-PAYBANK-" + turn + "-" + (Player[turn].getNoOfHouses() * 40) + (Player[turn].getNoOfHotels() * 115) + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 2:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 200 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 3:
                        ...
                        sound.playCash();
                        client.sendMessage("#-PAYBANK-" + turn + "-" + 100 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 4:
                        ...
                        sound.playCash();
                        client.sendMessage("#-PAYBANK-" + turn + "-" + 150 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 7:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 20 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 8:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 100 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 9:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 45 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 10:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 25 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 11:
                        ...
                        sound.playMoney();
                        client.sendMessage("#-PAYPLAYER-" + turn + "-" + 10 + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
                    ...
                    case 15:
                        ...
                        client.sendMessage("#-AQUIREDCCHESTCARD-" + turn + "-*");
                        try {
                            Thread.sleep(500);
                        } catch (Exception ae) {
                        }
    ...
    private void playIncomeTax() {
        ...
        pay.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                sound.playCash();
                client.sendMessage("#-PAYBANK-" + turn + "-" + 200 + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
    ...
    private void sendToJail() {
        ...
        ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                client.sendMessage("#-MOVEPLAYER-" + turn + "-10" + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
                ...
                client.sendMessage("#-INJAIL-" + turn + "-*");
                try {
                    Thread.sleep(500);
                } catch (Exception ae) {
                }
                ...
                for (int i = 0; i < 2; i++) {
                    if (i == 0) {
                        if (Player[turn].hasChanceJailCard()) {
                            ...
                            client.sendMessage("#-USECHANCECARD-" + turn);
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
                        ...
                        if (Player[turn].hasCommunityJailCard()) {
                            ...
                            client.sendMessage("#-USECOMMUNITYCARD-" + turn + "-*");
                            try {
                                Thread.sleep(500);
                            } catch (Exception ae) {
                            }
    ...
    private void playNoFunds(final int turn) {
        ...
        final JButton pay = new JButton("Trade");
...
// 																				makePlayersTab(Player);
    ...
    public void playRentPaid(final int payer, final int amount) {
        new Runnable() {
            ...
            public void run() {
                ...
                final JButton ok = new JButton("Ok");
                final JPanel p1 = new JPanel();
                addImage(p1, dir + "paidu.png", 350, 250);
                final JLabel token = new JLabel(Player[payer].getName(), Player[payer].token.getIcon(), JLabel.CENTER);
                final JLabel amt = new JLabel("$" + String.valueOf(amount));
                amt.setFont(new Font("cooper black", Font.BOLD, 30));
                JPanel Cback = new JPanel() {
                ...
                Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
                token.setBorder(raisedetched);
                token.setOpaque(true);
                p1.setBounds(15, 10, 350, 250);
                token.setBounds(150, 20, 180, 50);
                amt.setBounds(190, 170, 80, 40);
    ...
    public void playerMoved(final Player p) {
        try {
            new Runnable() {

                public void run() {
                    final JDialog Idialog = new JDialog();
                    final String dir = "images/street/";
                    //final JButton ok = new JButton("Ok");
                    final JPanel p1 = new JPanel();
                    final JPanel place = new JPanel();
                    final JLabel label = new JLabel(p.getName(), p.token.getIcon(), JLabel.CENTER);
                    final JLabel image = new JLabel(new ImageIcon(getClass().getResource("images/moved.png")));
                    Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
                    ...
                    image.setBorder(raisedetched);
                    place.setBorder(raisedetched);
                    addImage(place, dir + BoardComponent.images[p.getPosition()], 130, 180);
                    JPanel Cback = new JPanel() {
                    ...
                    Cback.add(image);
                    Cback.add(place);
                    label.setBounds(15, 10, 350, 40);
                    image.setBounds(15, 50, 350, 50);
                    place.setBounds(125, 110, 130, 180);
                    Idialog.setLayout(new GridLayout(1, 1));
                    Idialog.setBounds((int) (Board.getHeight() / 2) - 190, (int) (Board.getHeight() / 2) - 175, 380, 300);
                    Idialog.add(Cback);
                    Cback.setOpaque(false);
    ...
    public void purchasedProperty(final Player p) {
        new Runnable() {
            ...
            public void run() {
                ...
                final JPanel place = new JPanel();
                final JLabel label = new JLabel(p.getName(), p.token.getIcon(), JLabel.CENTER);
                final JLabel image = new JLabel(new ImageIcon(getClass().getResource("images/purchased.png")));
                Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
                //label.setHorizontalTextPosition(JLabel.CENTER);
                //label.setVerticalTextPosition(JLabel.BOTTOM);
                label.setFont(new Font("cooper black", Font.BOLD, 20));
                ...
                Cback.add(image);
                Cback.add(place);
                label.setBounds(15, 5, 350, 40);
                image.setBounds(15, 45, 350, 65);
                place.setBounds(125, 115, 130, 180);
                Idialog.setLayout(new GridLayout(1, 1));
                Idialog.setBounds((int) (Board.getHeight() / 2) - 190, (int) (Board.getHeight() / 2) - 175, 380, 300);
                Idialog.add(Cback);
                Cback.setOpaque(false);
    ...
    public void cardDrawn(final Player p, final int card, final int type) {
        new Runnable() {

            public void run() {
                final JDialog Idialog = new JDialog();
                final String dir = "images/chance/";
                final String dir2 = "images/Community_chest/";
                final JButton ok = new JButton("Ok");
                final JPanel p1 = new JPanel();
                final JPanel place = new JPanel();
                final JLabel label = new JLabel(p.getName(), p.token.getIcon(), JLabel.CENTER);
                final JLabel image = new JLabel(new ImageIcon(getClass().getResource("images/drawcard.png")));
                Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
                //label.setHorizontalTextPosition(JLabel.CENTER);
                //label.setVerticalTextPosition(JLabel.BOTTOM);
                label.setFont(new Font("cooper black", Font.BOLD, 20));
                ...
                place.setBorder(raisedetched);
                if (type == 1) {
                    addImage(place, dir + String.valueOf(card) + ".png", 250, 150);
                } else {
                    addImage(place, dir2 + String.valueOf(card) + ".png", 250, 150);
                }
                JPanel Cback = new JPanel() {
                ...
                Cback.add(place);
                label.setBounds(15, 10, 350, 40);
                image.setBounds(15, 50, 350, 65);
                place.setBounds(75, 130, 250, 150);
                Idialog.setLayout(new GridLayout(1, 1));
                Idialog.setBounds((int) (Board.getHeight() / 2) - 190, (int) (Board.getHeight() / 2) - 175, 380, 300);
                Idialog.add(Cback);
                Cback.setOpaque(false);
    ...
    public void houseAdded(int current, int no) {
        bank.purchaseHouse(Player[current], Cell[no], 1);
        houseimage[no].removeAll();
        if (no > 0 && no < 10) {
            if (Cell[no].getNumberOfHouses() == 1) {
    ...
    public void houseRemoved(int current, int no) {
        bank.removeHouse(Player[current], Cell[no], 1);
        houseimage[no].removeAll();
        if (no > 0 && no < 10) {
            if (Cell[no].getNumberOfHouses() == 1) {
    ...
    private class Time2 extends Thread {
        ...
        public void run() {
            try {
                ...
                for (int i = 0; i >= 0; i++) {
                    for (int j = 0; j < 60; j++) {
                        for (int k = 0; k < 60; k++) {
                            othertime.setText("TIME--> " + nf.format(hr) + ":" + nf.format(min) + ":" + nf.format(sec++));
    ...
    //MULTIPLAYER UPDATES
    public void moveToken(final Player token, final int position) {
        Cell[token.getPosition()].remove(token.token);
        Cell[token.getPosition()].revalidate();
        Cell[token.getPosition()].repaint();
        Cell[position].add(token.token);
        Cell[position].revalidate();
        Cell[token.getPosition()].revalidate();
        Cell[position].repaint();
    ...
    public void test() {
        Cell[1].setOwner(Player[0]);
        Cell[3].setOwner(Player[0]);
        Cell[11].setOwner(Player[0]);
        Cell[13].setOwner(Player[0]);
        Cell[14].setOwner(Player[0]);
